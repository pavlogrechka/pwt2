...
async function getCurrentAdminsData(adminName) {
  const receivedCurrentAdminsData = [];
  try {
    ...
    receivedCurrentAdminsData.push(result);
  } catch (err) {
    console.log(err);
  }

  await sql.close();
  return receivedCurrentAdminsData;
}

export { getCurrentAdminsData };
