...
const testInitConfig = {
  ...
  url:'https://pavlo-uat.uat.aptem.dev/pwa',
...
test('test1', async ({ page }) => {
  ...
  for await (let admin of receivedData) {
    ...
    const adminFound = page.locator('//table[@class="ng-star-inserted"]/tbody[1]/tr[1]/td[2]');
  ...
  const adminName = page.locator(`//tr[3]//img[@alt="user"]/following-sibling::span`);
  await expect(adminName).toContainText(`${receivedCurrentAdminsData[0].adminFullName}`);

  const currentCaseload = page.locator(`//tr[3]//div/div[2]/span`);
  ...
  const caseLoadCapacity = page.locator(`//tr[3]//div/div[3]/span`);
  ...
  for await (let admin of receivedData) {
    ...
    const adminName = page.locator(`//tr[${row}]//img[@alt="user"]/following-sibling::span`);
    await expect(adminName).toContainText(`${admin.adminFullName}`);

    // console.log(admin.adminCurrentCaseLoad);
    const currentCaseload = page.locator(`//tr[${row}]//div/div[2]/span`);
    ...
    const caseLoadCapacity = page.locator(`//tr[${row}]//div/div[3]/span`);
