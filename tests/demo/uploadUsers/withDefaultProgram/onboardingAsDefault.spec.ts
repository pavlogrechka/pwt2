...
test.only('Upload users', async ({ page }) => {
  ...
  await page.click(
    `//input[@value=${programs.delivery.parentGroupId}]/../../span`
  );
  ...
  await expect(locator).toContainText(
    'Imported: 0, Updated: 1, Canceled: 0, Failed: 0 '
  );
  ...
  const programNameLocator = await page.locator(
    `//td[text()="Programme"]/../th/span`
  );
