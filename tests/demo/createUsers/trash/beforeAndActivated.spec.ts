...
//   });
//   // const context = await browser.page()
//   await page.goto("https://mail.google.com/mail/u/0/#inbox");
//   await page.fill('//*[@name="identifier"]', "aptem1211");
//   await page.fill('//input[@type="password"]', "renepjdf2f");
// });
...
for (let user of usersList) {
  test(`create user ${startId++}`, async ({ page }) => {
    ...
    await page.fill(
      'input[name="UserContactInfoPart.ConfirmEmail"]',
      user.userEmail
    );
    await page.check("li:nth-child(6) .k-mid .k-checkbox-wrapper input");
    await Promise.all([
      page.waitForNavigation(/*{ url: 'https://pavlogrechka.test.aptem.co.uk/MWS.ClientAdmin/Users' }*/),
    ...
    await page.click(
      `//a[text()="${user.userFirstName} ${user.userLastName}"]`
    );
    console.log(`${user.userFirstName} ${user.userLastName}`);
    const spanWithUserName = page.locator(
      `span[@id="cphFilters_lblUserName" and contains(text(), ${user.userFirstName} ${user.userLastName})]`
    );
    await expect(spanWithUserName);
  });
}
