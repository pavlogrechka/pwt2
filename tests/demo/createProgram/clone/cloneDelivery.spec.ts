...
test('test', async ({ page }) => {
  ...
  await expect(page).toHaveURL(
    'https://pavlo-uat.uat.aptem.dev/MWS.PerformanceManager/ClientProgram'
  );
  ...
  await page.fill(
    locatorsClassic.clientProgram.programNameInput,
    testInitConfig.programName
  );
  await page.fill(
    locatorsClassic.clientProgram.programDescriptionTextArea,
    testInitConfig.programDescription
  );
  await page.click('//*[@aria-owns="Program_DeliveryType_listbox"]');
  await page.click(
    '//ul[@id="Program_DeliveryType_listbox"]/li[text()="Training - Apprenticeship"]'
  );
  await page.click('//div[@data-program-component="ILR"]');
  await page.click('//li/a[text()="Template"]');
  await page.click(
    '//div[@class="wizardForm-row"]//span[text()="Programme type:"]/..//span[@role="option"]'
  );
  await page.click('//li[text()="25 - Apprenticeship standard"]');
  await page.click(
    '//button[text()="Add new"]/../..//div[text()="List of aim references"]'
  );
  await page.click('#ilrForm >> text=Add new');
  await page.click(
    'text=Aim reference: Aim type: 1 - Programme aim3 - Component learning aim within a pr >> :nth-match(div, 4)'
  );
  await page.click('li:has-text("1 - Programme aim")');
  await page.click('#ilrForm >> :nth-match(:text("Add new"), 2)');
  await page.click(
    'text=Type: TNP - Total negotiated pricePMR - Payment recordADP - Additional Payment C >> :nth-match(div, 3)'
  );
  ...
  await page.click(
    'text=Name: See archived: Tags: Programme types: Search Reset >> :nth-match(div, 4)'
  );
  await page.click('li[role="option"]:has-text("Delivery")');
  await page.click(
    'text=DeliveryNoneOnboardingDeliverySub-programme >> span[role="button"]'
  );
