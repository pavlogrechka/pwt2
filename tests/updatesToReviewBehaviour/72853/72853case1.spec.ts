...
test('test1', async ({ page }) => {
  ...
  const startZoomMeetingBtn = '//button[text()=" Start Meeting "]'
  await page.waitForSelector(startZoomMeetingBtn, { timeout: testInitConfig.others.waitForSelectorTimeout })

  const returnToReviewsBtn = '//button[@aria-label="Return to reviews"]'
  await (await page.waitForSelector(returnToReviewsBtn, { timeout: testInitConfig.others.waitForSelectorTimeout })).click()

  const continueReviewBtn = `//tr/td/p[text()="${testInitConfig.createReview.reviewName}"]/../../td//button[text()="Continue"]`
  await (await page.waitForSelector(continueReviewBtn, { timeout: testInitConfig.others.waitForSelectorTimeout })).click()

  await (await page.waitForSelector(returnToReviewsBtn, { timeout: testInitConfig.others.waitForSelectorTimeout })).click()

  const editReviewBtn = `//tr/td/p[text()="${testInitConfig.createReview.reviewName}"]/../../td//button/label[text()="edit button"]`
  await (await page.waitForSelector(editReviewBtn, { timeout: testInitConfig.others.waitForSelectorTimeout })).click()

  const deleteReviewBtn = '//button//span[text()="Delete"]'
  await (await page.waitForSelector(deleteReviewBtn, { timeout: testInitConfig.others.waitForSelectorTimeout })).click()

  const confirmDeleteReviewBtn = '//button//span[text()=" Delete "]'
  await (await page.waitForSelector(confirmDeleteReviewBtn, { timeout: testInitConfig.others.waitForSelectorTimeout })).click()
